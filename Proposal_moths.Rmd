...
#Writing Floristic/Structural GLM Models for Arctiinae and Geometridae

```{r}
##Arctiid Diversity (Fisher's alpha) Structural Model
covar_moth <- read.csv("covar_moth.csv")

Arctiid.structural <- as.formula(Arctiine_FisherIndex ~ UnderComplex + CanopyCover + VerticalComplex)
Arctiid.structural.model <- glm(Arctiid.structural, data=covar_moth)
summary(Arctiid.structural.model)

##Geometrid Diversity (Fisher's alpha) Structural Model
Geometrid.structural <- as.formula(Geometrid_FisherIndex ~ UnderComplex + CanopyCover + VerticalComplex)
Geometrid.structural.model <- glm(Geometrid.structural, data=covar_moth)
summary(Geometrid.structural.model)

##Arctiid Diversity (Fisher's alpha) Floristic Model
Arctiid.floristic <- as.formula(Arctiine_FisherIndex ~ VegDiversity + VegDensity + NMDS1 + NMDS2)
Arctiid.floristic.model <- glm(Arctiid.floristic, data=covar_moth)
summary(Arctiid.floristic.model)

##Geometrid Diversity (Fisher's alpha) Floristic Model
Geometrid.floristic <- as.formula(Geometrid_FisherIndex ~ VegDiversity + VegDensity + NMDS1 + NMDS2)
Geometrid.floristic.model <- glm(Geometrid.floristic, data=covar_moth)
