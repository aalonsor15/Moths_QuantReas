...
correlations <- cor(Mothveg,use="complete.obs", method="pearson")
```
We used the Pearson and Kendall methods to determine correlation between the vegetation parameters. We determined that a floristic model could safely include the following parameters since they lack strong correlations: plant species diversity, vegetation density, and a plant composition matrix. We also found that a structural model could include the following uncorrelated parameters: understory complexity, canopy coverage, and vertical complexity. 
